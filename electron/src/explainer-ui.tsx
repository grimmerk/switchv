...
const ExplainerApp: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleCodeToExplain = (_event: any, receivedCode: string) => {
      console.log('Received code to explain, length:', receivedCode?.length || 0);
        ...
        <div style={styles.explanation}>
          <ReactMarkdown
            ...
            components={{
              ...
              p: ({children}) => <p style={{marginTop: '0.5em', marginBottom: '0.5em'}}>{children}</p>,
              ul: ({children}) => <ul style={{paddingLeft: '20px'}}>{children}</ul>,
              ol: ({children}) => <ol style={{paddingLeft: '20px'}}>{children}</ol>,
              li: ({children}) => <li style={{margin: '0.2em 0'}}>{children}</li>,
              h1: ({children}) => <h1 style={{fontSize: '1.5em', marginTop: '0.8em', marginBottom: '0.5em'}}>{children}</h1>,
              h2: ({children}) => <h2 style={{fontSize: '1.3em', marginTop: '0.8em', marginBottom: '0.5em'}}>{children}</h2>,
              h3: ({children}) => <h3 style={{fontSize: '1.1em', marginTop: '0.8em', marginBottom: '0.5em'}}>{children}</h3>,
              blockquote: ({children}) => <blockquote style={{borderLeft: '4px solid #444', paddingLeft: '10px', margin: '0.5em 0'}}>{children}</blockquote>,
            }}
          >
            {explanation}
          </ReactMarkdown>
