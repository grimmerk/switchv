...
// const t = process.env.PRISMA_QUERY_ENGINE_BINARY;
// console.log({t})
...
console.log({SERVER: process.env.EMBEDSERVER}); 
...
(async ()=>{
  ...
  // const needMigrateToVer = await DBManager.checkNeedMigration();

  // 使用 server process 去 migrate, 應該是起來時就因為收到的參數來決定要不要 migrate, 
  // 主要是因為 electron version 相關資訊要在這裡做
  // 完成後再去更新 dbVersion 
